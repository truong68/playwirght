    ...
    async with async_playwright() as p:
        ...
        try:
            for idx, row in df.iterrows():
                ...
                # ================== BƯỚC 2: BẤM "THÊM ẢNH MỚI" + UP ẢNH TỪ THƯ MỤC ANH + BẤM ĐĂNG ==================
                print(f"[{phone}] Đang bấm Thêm ảnh mới và upload ảnh...")
                try:
                    await page.wait_for_selector('p:has-text("Thêm ảnh mới")', timeout=10000)

                    add_photo_clicked = False
                    for sel in [
                        'p:has-text("Thêm ảnh mới")',
                        'div.border-dashed.border-\\[2px\\].border-\\[\\#E3E2E2\\] p',
                        'div.h-\\[240px\\] p:text("Thêm ảnh mới")'
                    ]:
                        try:
                            await page.eval_on_selector(sel, "el => el.scrollIntoViewIfNeeded()")
                            await asyncio.sleep(0.3)
                            await page.click(sel, timeout=8000, force=True)
                            print(f"[{phone}] ĐÃ BẤM THÀNH CÔNG VÀO NÚT THÊM ẢNH MỚI!")
                    ...
                    if not add_photo_clicked:
                        print(f"[{phone}] Không tìm thấy nút Thêm ảnh mới")
                    else:
                        # UP ẢNH TỪ THƯ MỤC ANH
                        import os
                        import random
                        ...
                        if os.path.exists(IMAGE_FOLDER):
                            ...
                            if images:
                                ...
                                await asyncio.sleep(5)  # đợi ảnh load xong

                                # TỰ ĐỘNG BẤM NÚT "ĐĂNG"
            ...
            print(f"\nHOÀN TẤT! Đã đăng bài thành công cho {total} tài khoản (có nội dung + ảnh + tự động ĐĂNG)!")
